            ...
            <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70">
                ...
                <div className="mb-6">
                    ...
                    <div className="relative flex justify-between items-start mt-2">
                        ...
                        <button onClick={() => setShowAddress(!showAddress)} style={{color: 'var(--color-primary)',cursor: 'pointer', textDecoration: isHovered ? 'underline' : 'none'}} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
                            Change
                        </button>
                        {showAddress && (
                            <div className="absolute top-12 py-1 bg-white border border-gray-300 text-sm w-full">
                                {addresses.map((address,index) =>(<p key={address._id || index}   onClick={() => {setSelectedAddress(address); setShowAddress(false)}} className="text-gray-500 p-2 hover:bg-gray-100">
                                    {address.street},{address.city},{address.state},{address.country}
                                </p>
                            ))}
                                <p onClick={() => navigate("/add-address")} style={{color: 'var(--color-primary)',textAlign: 'center',cursor: 'pointer',padding: '0.5rem',backgroundColor: hovered ? 'rgba(var(--color-primary-rgb), 0.1)' : 'transparent',}} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
                                    Add NEW Address
                                </p>
                            </div>
                        )}
